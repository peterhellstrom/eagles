...
# 2) Calculate totals and sub-totals for all hierarchical levels (use cube function in data.table)
# 3) Calculate derived variables (wtse_vars)
# wtse_sum is a wrapper function that performs all steps
...
#' @export
wtse_transform <- function(.data, ..., .columns = NULL, .drop = FALSE) {
  # ... are the user-specified grouping variables, e.g. Region, CensusYear
  ...
  if (missing(.columns)) .columns <- wtse_columns()

  .data_pivot <- .data %>%
    count(..., UtfallR, .drop = .drop) %>%
    pivot_wider(values_from = n, names_from = UtfallR)

  .columns %>%
    bind_rows(.data_pivot) %>%
    relocate(...) %>%
    replace(., is.na(.), 0)
}
...
#' @export
wtse_vars <- function(.data, clean_names = FALSE) {
  .x <- .data %>%
    mutate(
      `inventerade_revir` = rowSums(select(.,`ej_h`:`annan_art`)),
      `besatta_revir` = rowSums(select(.,`ej_h`:`ad_i_revir`)),
      `ej_besatta_revir` = rowSums(select(.,`observation`:`annan_art`)),
      `antal_par` = rowSums(select(.,`ej_h`:`par_ej_hack`)),
      `bebott_bo` = rowSums(select(.,`ej_h`:`h_okant_res`)),
      `produktiva_par` = rowSums(select(.,`lh_1`:`lh_min_3`)),
      `improduktiva_par` = rowSums(select(.,`ej_h`:`missl`)),
...
#' @export
wtse_sum <- function(.data, ..., .columns = NULL, .remove = FALSE) {

  if (missing(.columns)) .columns <- wtse_columns()

  .x <- .data %>%
    wtse_transform(..., .columns = .columns)

  .x <- .x %>%
    data.table::data.table() %>%
    data.table::cube(lapply(.SD, sum), by = sapply(vars(...), rlang::quo_text)) %>%
    as_tibble() %>%
    wtse_vars()
  ...
  if (.remove) {
    .x <- .x %>% select(-c(`ej_h`:`uppgift_saknas`))
  }
