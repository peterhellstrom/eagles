...
sweref99_add_prefix <- function(.x, crs) {
  ...
  crs_codes <- c("TM", "1200", "1330", "1500", "1630", "1800", "1415", "1545",
...
sweref99_index <- function(
    .x, .y = NULL, .grid_size, crs = 3006) {

  # Allowed grid sizes
  gs <- c(100, 50, 25, 10, 5, 2.5, 1, 0.5, 0.25,
  ...
  index_main <- str_c(floor(.y / g_up), floor(.x / g_up), sep = "_")
...
add_sweref99_index <- function(
    ...
    crs = st_crs(data)$epsg,
    .prefix = "grid") {

  .data %>%
    bind_cols(
      map(
        .grid_size, ~ sweref99_index(data, .grid_size = .x, crs = 3006)) %>%
...
sweref99_index_alphanum <- function(
  ...
  an_100 <- str_c(floor(.y / 100000), LETTERS[floor(1 + (.x / 100000))])
  ...
  an_10 <- str_c(floor((.y %% 100000) / 10000), letters[floor(1 + ((.x %% 100000) / 10000))])

  if (.grid_size == 100000) {
    out <- an_100
  } else if (.grid_size == 50000) {
    an_50 <- quadrant_cardinal(floor((.y %% 100000) / 50000), floor((.x %% 100000) / 50000))
    out <- str_c(an_100, an_50, sep = " ")
  } else if (.grid_size == 10000) {
    if (fastighetsblad == FALSE) {
      out <- str_c(an_100, an_10, sep = " ")
    } else {
      out <- str_c(an_100, an_10,
                   if_else(floor((.y %% 10000) / 5000) == 0, "S", "N"),
                   sep = " ")
    }
  } else if (.grid_size == 5000) {
    an_5 <- quadrant_cardinal(floor((.y %% 10000) / 5000), floor((.x %% 10000) / 5000))
    out <- str_c(an_100, an_10, an_5, sep = " ")
  } else if (.grid_size == 1000) {
    an_1 = str_c(floor((.y %% 10000) / 1000), letters[floor(1 + ((.x %% 10000) / 1000))])
    out <- str_c(an_100, an_10, an_1, sep = " ")
  } else {
    out <- NA
  }
