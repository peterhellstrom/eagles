...
# pch = point character symbol for time series points, pch.col=color of point character symbol
# pch.bg = optional background color of point character symbols (used e.g. if pch=21)
# add = FALSE/TRUE, add.points = FALSE/TRUE, add.line = FALSE/TRUE
# ref.line = FALSE/TRUE: draw reference level lines, as specified by argument reference
# ref.col = color of reference lines, ref.lty = line type vector for reference level lines,
# ref.lwd = line width vector for reference lines
# trend.col =
# trend.lty =
# trend.lwd
# signif =
...
# REPLACE: GAM -  check packages gratia (fderiv, derivatives, confint) and itsadug
# OLD: confint, signifD, Deriv, derivSimulCI

#' @export
wtse_trend <- function(y, x, model,
	reference = c(0, 0, 0), npts = 500, alpha = 0.05,
	pch = 21, pch.col = "black", pch.bg = "steelblue", pch.cex = 1,
	add = FALSE, add.points = TRUE, add.line = TRUE,
	ref.line = TRUE, ref.col = "darkgrey", ref.lty = c(3, 2, 3), ref.lwd = c(1.5, 1.5, 1.5),
	ref.polygon = FALSE, ref.polygon.col = "yellow",
	trend.col = "black", trend.lty = 1, trend.lwd = 1.5,
	signif = TRUE, signif.lwd = 2, signif.col = c("red", "blue"),
	ci = TRUE,
	main = "White-tailed sea eagle", xlab = "Year", ylab = "Variable", ...) {
	...
	if (class(m)[1] == "loess") {
		#pdat <- with(m, data.frame(seq(min(m[[cn]]), max(m[[cn]]), length = npts)))
		pdat <- with(m, data.frame(seq(min(m[["x"]]), max(m[["x"]]), length = npts)))
	} else {
		pdat <- with(m$model, data.frame(seq(min(m$model[,cn]), max(m$model[,cn]), length = npts)))
	}
	names(pdat) <- cn

	# Predict model
	p <- predict(m, newdata = pdat, type = "response")
	# Confidence intervals
	p_ci <- predict(m, newdata = pdat, se = TRUE, type = "response")
	# For GAM-models, find "significant" parts
	if (class(model)[1] == "gam" | class(model)[1] == "gamm") {
		# calculate first derivative of model
		m.d <- gratia::derivatives(model, n = npts)
		S <- with(m.d, signifD(p, derivative, upper, lower, eval = 0))
	} else {
	...
	if (add == FALSE) plot(x, y, type = "n", bty = "l", main = main, xlab = xlab, ylab = ylab, ...)
	...
	if (ref.polygon) {
		...
		col = ref.polygon.col, border = NA)
	}

	if (ref.line) abline(h = reference, lty = ref.lty, lwd = ref.lwd, col = ref.col)

	if (add.line) {
		lines(p ~ get(cn), data = pdat, lty = trend.lty, lwd = trend.lwd, col = trend.col)
		if (signif) {
			lines(S$incr ~ get(cn), data = pdat, lwd = signif.lwd, col = signif.col[2])
			lines(S$decr ~ get(cn), data = pdat, lwd = signif.lwd, col = signif.col[1])
		}
		if (ci) {
			lcl <- p_ci$fit - 1.96 * p_ci$se.fit
			ucl <- p_ci$fit + 1.96 * p_ci$se.fit
			lines(lcl ~ get(cn), data = pdat, lty=2)
			lines(ucl ~ get(cn), data = pdat, lty=2)
		}
	}

	if (add.points) points(y ~ x, pch = pch, col = pch.col, bg = pch.bg, cex = pch.cex)
}
