...
kolumner <- readxl::read_excel(
  file.path("..","..","kungsorn_rapporter_info.xlsx"),
  sheet = "kolumner",
  range = "A1:K112")

# Kolla Rovbase! Stämmer ordning etc.
besok_status <- tribble(
  ~Status, ~SortOrder, ~Aktivitet,
  # Svensk metodik
...
rovbase_reports <- tribble(
  ~report_nr, ~report_id, ~instruks, ~report_name_rovbase, ~report_short_name, ~obligatory, ~n_columns, ~koordinater,
  ...
  6, "A06", "sv", "(S) Kongeørn årspost, besøk - svensk instruks", "arspost_besok_sv", FALSE, 54, TRUE,
  7, "A07", "no", "(S) Kongeørn årspost, besøk, reirplass - norsk instruks", "arspost_besok_boplats_no", FALSE, 76, TRUE,
  8, "A08", "sv", "(S) Kongeørn årspost, besøk, reirplass - svensk instruks", "arspost_besok_boplats_sv", FALSE, 78, TRUE,
  9, "A09", "no", "(S) Kongeørn, norsk instruks", "arspost_no", TRUE, 28, FALSE,
  10, "A10", "sv", "(S) Kongeørn, svensk instruks", "arspost_sv", TRUE, 22, FALSE,
