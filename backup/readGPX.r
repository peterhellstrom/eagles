...
# Note           : corrections in the code by: Berry Boessenkool (berryboessenkool@hotmail.com), updated by Peter Hellström 2018-11-25
...
.readGPX.element <- function(gpx.file, element) {
    ...
    if(any(grep(element, names(top)))) {
	  ...
	  nmv <- unlist(lapply(top[nu], names), use.names = FALSE)
	  nm <- unique(nmv)
	  nm <- setNames(nm, nm)
	  nm <- nm[match(c("name", "ele", "time", "sym", "cmt", "desc", "type", "extensions"), nm)]
	  # Changes stops here
      #nm <- names(top[[nu[1]]]) # Original code
